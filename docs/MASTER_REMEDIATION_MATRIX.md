...
| ID | File | Severity | Finding | Required Fix | Test Required | Status / Verification |
|----|------|----------|---------|--------------|---------------|-----------------------|
| P1-01 | `server/security/rateLimiter.ts:56` | P1 | **PostgreSQL rate limiter uses `queryRaw` without index hint** — `WHERE "id" = ${windowId} AND "count" < ${limit}` may scan | Add partial index on `(id) WHERE count < limit` | Performance: EXPLAIN ANALYZE shows index usage | Open |
| P1-02 | `server/queue/jobQueue.ts:290` | P1 | **Lease interval uses string interpolation** — `${DEFAULT_LEASE_MS} || ' milliseconds'::interval` SQL injection risk via template literal | Use parameterized query or cast | Security: SQL injection test with malicious workerId | Open |
...
| P1-04 | `server/queue/worker.ts` | P1 | **Worker uses in-memory JobQueueManager** — `import { JobQueueManager } from './jobQueue'` | Switch worker to PrismaQueueAdapter in production | Integration: worker processes jobs from PG queue | Open |
| P1-05 | `server/services/ai.service.ts:107` | P1 | **OpenAI API key not validated at startup** — fails at runtime only | Validate `OPENAI_API_KEY` at startup; fail fast | Unit: startup fails without OPENAI_API_KEY | Open |
| P1-06 | `server/services/paymentService.ts:91` | P1 | **Stripe signature parsing expects `t=...` format** — doesn't handle v1/v2/v3 variations | Parse per Stripe spec; support multiple v1 entries | Unit: Stripe webhook signature test vectors | **STATUS: FIXED** (Verified in Section 4) |
| P1-07 | `server/security/firebaseAuth.ts:59` | P1 | **Firebase cert cache never expires on rotation** — `cachedCerts` TTL based on cache-control max-age | Add max TTL cap (1hr); force refresh on verify failure | Integration: cert rotation test | Open |
| P1-08 | `server/security/firebaseAuth.ts:51` | P1 | **Firebase project ID from config file** — `firebase-applet-config.json` checked into repo | Move to env var; remove from repo | Security: repo scan for secrets | Open |
| P1-09 | `server/services/paymentService.ts` | P1 | **Cashfree verification uses base64 HMAC** — signature in `x-webhook-signature` header | Verify Cashfree v3 spec; add tolerance window | Unit: Cashfree webhook test vectors | **STATUS: FIXED** (Verified in Section 4) |
| P1-10 | `server/scanner/detectors/security.ts` | P1 | **Security detector doesn't check CSP headers** — CSP misconfiguration not detected | Add CSP header analysis | Unit: CSP header test cases | Open |
| P1-11 | `server/scanner/detectors/seo.ts` | P1 | **SEO detector doesn't check canonical URL** — missing canonical URL not flagged | Add canonical URL check | Unit: SEO test with missing canonical | Open |
| P1-11 | `server/scanner/detectors/performance.ts` | P1 | **Performance detector doesn't check image optimization** — large unoptimized images not flagged | Add image size/format checks | Unit: performance test with oversized images | Open |
