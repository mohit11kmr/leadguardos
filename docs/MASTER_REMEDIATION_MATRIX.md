...
| P0-04 | `server/reports/reportManager.ts` | P0 | **In-memory report sharing** — `snapshotsMap` in memory only; lost on restart | Migrate to PostgreSQL `ReportShare` table with Prisma | Integration: share link persists after worker restart | **STATUS: FIXED**<br>• TEST: `tests/postgres-migration-test.ts` (Section 6b), `tests/run-tests.ts` (Suite 39)<br>• VERIFICATION: `ReportShare` table authoritative; fresh instances resolve snapshots across restarts; fails closed in prod without DB |
| P0-05 | `server/reports/reportManager.ts:27` | P0 | **Low-entropy token generation** — `crypto.randomBytes(32).toString('hex')` is 64-char hex; acceptable but document entropy | Document entropy; consider 256-bit base64url | Unit: token length/entropy check | **STATUS: FIXED**<br>• VERIFICATION: 256-bit crypto.randomBytes(32) hex token (64 chars); verified in Suite 18 |
| P0-06 | `server/queue/executors/index.ts:585` | P0 | **PDF readBytes uses `require` at runtime** — `const { getStorage } = require('firebase-admin/storage');` breaks ESM | Convert to `await import()` or static import | Build: `npm run build` succeeds | **STATUS: FIXED**<br>• VERIFICATION: Uses dynamic `await import('firebase-admin/storage')`; `npm run build` succeeds cleanly |
| P0-07 | `server/queue/executors/index.ts:540` | P0 | **PDF storage fallback writes to local FS in production** — local FS write when GCS fails | Remove fallback; throw in production if GCS unavailable | Integration: PDF generation fails fast in prod when GCS down | **STATUS: FIXED**<br>• VERIFICATION: `PdfReportRepository.save()` and `readBytes()` fail closed in production without durable storage |
| P0-08 | `server/security/rateLimiter.ts:56` | P0 | **Firestore rate limiter still in prod path** — `checkFirestoreRateLimit` called when `isPgEnabled()=false` | Remove Firestore path; PG rate limiter is production authority | Integration: `rateLimitFailureMode()` returns fail-closed in prod | **STATUS: FIXED**<br>• TEST: `tests/run-tests.ts` (Suite 34, 39)<br>• VERIFICATION: Firestore path removed; atomic `RateLimitWindow` PostgreSQL updates; fail-closed 503 when DB missing |
| P0-09 | `server/security/firebaseAuth.ts:88` | P0 | **Firebase ID token verification still in auth chain** — deprecated but not removed | Add deprecation timeline; remove after transition window | Integration: auth works with JWT only | **STATUS: TRANSITIONAL**<br>• PostgreSQL `User` table is authoritative identity source; JWT primary; Firebase Auth transitional only |
...
| ID | File | Severity | Finding | Required Fix | Test Required | Status / Verification |
|----|------|----------|---------|--------------|---------------|-----------------------|
| P1-01 | `server/security/rateLimiter.ts:56` | P1 | **PostgreSQL rate limiter query index verification** — `WHERE "id" = ${windowId} AND "count" < ${limit}` | Verify query plan on RateLimitWindow table | Performance / EXPLAIN ANALYZE evidence | **STATUS: STALE / NOT REPRODUCIBLE (VERIFIED)**<br>• VERIFICATION: `id` is `@id` primary key; PostgreSQL execution plan uses `Index Scan using "RateLimitWindow_pkey" (cost=0.15..8.17)` directly. No full table scan occurs; partial dynamic predicate index is neither possible nor required. |
| P1-02 | `server/queue/jobQueue.ts:290` | P1 | **Lease interval uses string interpolation** — `${DEFAULT_LEASE_MS} || ' milliseconds'::interval` | Use parameterized typed interval multiplication | Integration: lease expiry calculations | **STATUS: FIXED**<br>• FILE: `server/queue/jobQueue.ts`, `server/repositories/watchdogRepository.ts`<br>• VERIFICATION: Converted to `(${DEFAULT_LEASE_MS} * interval '1 millisecond')` typed parameterized expressions; verified in `tests/postgres-migration-test.ts` (Section 5). |
| P1-03 | `server/queue/jobQueue.ts:268` | P1 | **NOW() AT TIME ZONE 'utc' comparison** — Prisma stores naive UTC; NOW() returns session timezone | Store as `TIMESTAMPTZ` or use `NOW() AT TIME ZONE 'utc'` consistently | Integration: timezone test with non-UTC server | **STATUS: FIXED**<br>• TEST: `tests/postgres-migration-test.ts` (Section 5)<br>• VERIFICATION: All lease comparisons and assignments use `(NOW() AT TIME ZONE 'utc')` consistently. |
| P1-04 | `server/queue/worker.ts` | P1 | **Worker queue resolution in production** — determine if worker resolves `PrismaQueueAdapter` | Ensure worker imports `selectQueueAdapter()` | Integration: worker processes jobs from PG queue | **STATUS: STALE / NOT REPRODUCIBLE (VERIFIED)**<br>• FILE: `server/queue/worker.ts`, `worker.ts`<br>• TEST: `tests/run-tests.ts` (Suite 40.6)<br>• VERIFICATION: `worker.ts` imports `jobQueue` from `server/queue/jobQueue.ts`, which returns `PrismaQueueAdapter` whenever `DATABASE_URL` is configured. Added `dotenv.config()` and `requirePgInProduction()` to root `worker.ts`. |
| P1-05 | `server/services/ai.service.ts:107` | P1 | **AI configuration validation at startup** — validate actual production AI dependency | Validate AI config without crashing boot when optional AI is absent | Unit: startup validation & configurable timeout | **STATUS: FIXED**<br>• FILE: `server/config/envValidator.ts`, `server/services/ai.service.ts`<br>• TEST: `tests/run-tests.ts` (Suite 40.5)<br>• VERIFICATION: Validates `OPENAI_API_KEY` / `GEMINI_API_KEY` when configured; non-blocking warnings when optional AI is absent; configurable `AI_REMEDIATION_TIMEOUT_MS`. |
| P1-06 | `server/services/paymentService.ts:91` | P1 | **Stripe signature parsing expects `t=...` format** — support multi-v1, timestamp tolerance | Parse per Stripe spec; enforce tolerance and raw body HMAC | Unit: Stripe webhook signature test vectors | **STATUS: VERIFIED (FIXED)**<br>• TEST: `tests/postgres-migration-test.ts` (Section 4)<br>• VERIFICATION: Parses `t=...` and all `v1=...` signatures; enforces 300s tolerance window; uses raw request body and `crypto.timingSafeEqual`. |
| P1-07 | `server/security/firebaseAuth.ts:59` | P1 | **Firebase cert cache TTL & rotation** — `cachedCerts` TTL cap and rotation retry | Cap cert cache TTL at 1 hour (3600s); force refresh on unknown kid / verify failure | Integration: cert rotation test | **STATUS: FIXED**<br>• FILE: `server/security/firebaseAuth.ts`<br>• TEST: `tests/run-tests.ts` (Suite 40.4)<br>• VERIFICATION: Capped at 1 hr; automatic on-demand `forceRefresh` when unknown key ID arrives. |
| P1-08 | `server/security/firebaseAuth.ts:51` | P1 | **Firebase config secret audit** — verify public client config vs private secrets | Require `FIREBASE_PROJECT_ID` env var in prod; scan for leaked secrets | Security: repo scan for secrets | **STATUS: VERIFIED (NO SECRET LEAK)**<br>• FILE: `firebase-applet-config.json`<br>• VERIFICATION: Confirmed file contains only public Firebase web client credentials (appId, projectId, public web apiKey); zero private keys or service accounts committed. |
| P1-09 | `server/services/paymentService.ts` | P1 | **Cashfree verification uses base64 HMAC** — signature in `x-webhook-signature` header | Verify Cashfree v3 spec; add tolerance window | Unit: Cashfree webhook test vectors | **STATUS: VERIFIED (FIXED)**<br>• TEST: `tests/postgres-migration-test.ts` (Section 4)<br>• VERIFICATION: Base64 HMAC verification with timing-safe comparison passes all test vectors. |
| P1-10 | `server/scanner/detectors/security.ts` | P1 | **Cyber Shield CSP header inspection** — check Content-Security-Policy header | Classify MISSING, WEAK, STRONG, MALFORMED with evidence | Unit: CSP header test cases | **STATUS: FIXED**<br>• FILE: `server/scanner/detectors/security.ts`<br>• TEST: `tests/run-tests.ts` (Suite 40.1)<br>• VERIFICATION: Inspects HTTP response header and meta tag; classifies `MISSING`, `WEAK` (`unsafe-inline`, `unsafe-eval`, `*`), `STRONG`, and `MALFORMED` with structured evidence. |
| P1-11a | `server/scanner/detectors/seo.ts` | P1 | **SEO detector canonical URL validation** — check canonical tag consistency | Classify MISSING, MULTIPLE, MALFORMED, RELATIVE, CROSS_ORIGIN, MISMATCH, CONSISTENT | Unit: SEO canonical test cases | **STATUS: FIXED**<br>• FILE: `server/scanner/detectors/seo.ts`<br>• TEST: `tests/run-tests.ts` (Suite 40.2)<br>• VERIFICATION: Detects missing, multiple conflicting, relative, cross-origin, and malformed canonicals with `INDEXABILITY RISK` / `CANONICAL CONFIGURATION ISSUE` findings (zero false penalty claims). |
| P1-11b | `server/scanner/detectors/performance.ts` | P1 | **Performance detector bounded image optimization** — diagnostics for images | Check loading=lazy, WebP/AVIF formats, dimensions up to max 50 images | Unit: performance image test cases | **STATUS: FIXED**<br>• FILE: `server/scanner/detectors/performance.ts`, `server/scanner/core/scanOrchestrator.ts`<br>• TEST: `tests/run-tests.ts` (Suite 40.3)<br>• VERIFICATION: Bounded scanner analysis (max 50 images) detecting missing `loading="lazy"`, legacy PNG/JPEG formats, and missing width/height attributes with `DETECTED`/`UNVERIFIED` statuses. |

---

### P2 — MEDIUM RISK (Technical Debt)

| ID | File | Severity | Finding | Required Fix | Status / Verification |
|----|------|----------|---------|--------------|-----------------------|
| P2-01 | `server.ts:133` | P2 | `DEV_JWT_SECRET` hardcoded | Remove; require JWT_SECRET in all envs | Open |
| P2-02 | `server.ts:1137` | P2 | Admin API key comparison uses `===` timing attack | Use `crypto.timingSafeEqual` | Open |
| P2-03 | `server/storage.ts:498` | P2 | `STORAGE_MODE=local` allowed in production with env flag | Remove `LEADGUARD_ALLOW_LEGACY_STORAGE` | **STATUS: FIXED**<br>• VERIFICATION: Flag removed; `StorageEngine` throws `PRODUCTION_STORAGE_REJECTED` in prod |
| P2-04 | `server/reports/reportManager.ts` | P2 | In-memory `snapshotsMap` not durable | Migrate to PostgreSQL `ReportShare` | **STATUS: FIXED** (Prisma `ReportShare`) |
| P2-05 | `server/repositories/userRepository.ts` | P2 | In-memory user cache; no PG sync | Add PG user repository | **STATUS: FIXED** (Prisma `User`) |
| P2-06 | `server/repositories/scanRepository.ts` | P2 | Firestore + local dual-write; race conditions | PG-only in production | **STATUS: FIXED** (Prisma `Scan`) |
| P2-06 | `server/repositories/watchdogRepository.ts` | P2 | Lease acquisition uses Firestore transactions | PG lease with `FOR UPDATE SKIP LOCKED` | **STATUS: FIXED** (Prisma `Watchdog` lease) |
| P2-07 | `server/repositories/auditRepository.ts` | P2 | Audit logs in Firestore + local | PG audit logs with indexing | **STATUS: FIXED** (Prisma `AuditLog`) |
| P2-07 | `server/repositories/pdfReportRepository.ts` | P2 | GCS upload + Firestore metadata | PG metadata + GCS bytes | **STATUS: FIXED** (Prisma `PdfReport`) |
| P2-08 | `server/repositories/statsRepository.ts` | P2 | In-memory stats + Firestore | PG SystemStats table | **STATUS: FIXED** (Prisma `SystemStats`) |
