...
function computeDeliveryKey(event: string, recipient: string, provider: string): string {
...
export async function executeGeneratePdf(job: QueueJobPayload): Promise<PdfMetadata> {
  ...
  const pdfBytes: Uint8Array = doc.output('arraybuffer');
  ...
  try {
    ...
    // If Firebase Storage is not available, write to local filesystem as fallback
    if (process.env.NODE_ENV !== 'production') {
      const fs = require('fs');
  ...
  // Persist metadata to Firestore — MUST succeed
  const { isFirebaseConfigured, getAdminDb, FieldValue } = require('../../firebaseAdmin');
  if (isFirebaseConfigured()) {
...
export async function executeRunWatchdog(job: QueueJobPayload): Promise<any> {
  ...
  const scanResult = await executeLiveWebsiteScan(target.targetUrl);
  const status = (scanResult.overallScore ?? scanResult.score ?? 0) >= 70 ? 'HEALTHY' : 'INCIDENT_OPEN';
